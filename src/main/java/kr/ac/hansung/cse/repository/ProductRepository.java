...
public class ProductRepository {
    ...
    public List<Product> findByNameAndCategory(String keyword, Long categoryId) {
        return entityManager.createQuery(
                        "SELECT p FROM Product p JOIN FETCH p.category WHERE p.name LIKE :keyword AND p.category.id = :cid",
                        Product.class)
                .setParameter("keyword", "%" + keyword + "%")
                .setParameter("cid", categoryId)
                .getResultList();
